...
# For each epoch
    for epoch_idx in range(start_epoch, end_epoch):
        ...
        for site in range(len(sites)):
            ...
            if epoch_idx == 0:
                model_prev_ep = torch.load(global_dir + '/Initial_weights.pt')['model']
                delta_i    = model.state_dict() - model_prev_ep

            else:
                model_prev_ep = torch.load('/ckpt_epoch' +str(epoch_idx-1)+  'site'+str(sites[site])+'after_share_weights.pt')['model_state_dict']
                delta_i   = model.state_dict() - model_prev_ep
        ...
        #calculate relavent adaptive params:
        # delta_t = weighted average of changes in model weights for each sites
        with torch.no_grad():
            if epoch_idx == 0:
                ag_delta = 0
                for site in sites:
                    ag_delta = ag_delta + torch.load('/epoch'+str(epoch_idx)+'_site'+str(sites[site])+'last_weights.pt')['delta_i']
                delta_t = beta1*0 + ((1-beta1)/len(sites))*ag_delta_i
            else:
                ag_delta = 0
                for site in sites:
                    ag_delta = ag_delta + torch.load('/epoch'+str(epoch_idx)+'_site'+str(sites[site])+'last_weights.pt')['delta_i']
                delta_t = beta1*torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx-1)'_after_share_weights.pt'))['delta_t'] + (1-beta1)/len(sites)*ag_delta_i

            if Adaptive_alg == 'FedAdaGrad':
                v_t = torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx-1)'_after_share_weights.pt'))['v_t'] + delta_t**2
            elif Adaptive_alg == 'FedYogi':
                v_t = torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx-1)'_after_share_weights.pt'))['v_t'] - (1-beta2)*delta_t**2 * (sign(torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx-1)'_after_share_weights.pt'))['v_t']-delta_t**2))
            elif Adaptive_alg == 'FedAdam':
                v_t = beta2 * torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx-1)'_after_share_weights.pt'))['v_t']+(1-beta2)*delta_t**2

            final_weights = torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx-1)'_after_share_weights.pt') + eta * delta_t/(v_t + tau)
                ...
                torch.save({
                    'epoch': epoch_idx,
                    'sample_idx': sample_idx,
                    'model_state_dict': final_weights.state_dict(),
                    'delta_t': delta_t,
                    'v_t': v_t,
                    'hparams': hparams,
