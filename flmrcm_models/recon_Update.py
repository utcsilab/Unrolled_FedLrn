...
class LocalUpdate_ad_da(object):
    ...
    def __init__(self, args, device, dataset, dataset_target,optimizer,optimizer_fd,flag):
        ...
        self.selected_clients = []
        self.ldr_train = DataLoader(dataset, batch_size=1, shuffle=True, pin_memory=True)
        self.target_domain = DataLoader(dataset_target, batch_size=1, shuffle=True, pin_memory=True)
    ...
    def train(self, net, net_fd, epoch, idx, writer, logs):
        net.train()
        net_fd.train()
        # train and update
        ...
        epoch_loss_adv_g = []
        epoch_loss_adv_d = []
        for iter in range(self.args.share_int):
            batch_loss = []
            batch_loss_L1 = []
            batch_loss_adv_g = []
            batch_loss_adv_d = []
        ...
        logs['L1'].append(sum(epoch_loss_L1) / len(epoch_loss_L1))
        logs['total_loss'].append(sum(epoch_loss) / len(epoch_loss))
        logs['adv_g'].append(sum(epoch_loss_adv_g) / len(epoch_loss_adv_g))
        logs['adv_d'].append(sum(epoch_loss_adv_d) / len(epoch_loss_adv_d))
        
        writer.add_scalar('TrainLoss/L1/'+ str(self.args.client_sites[idx]), sum(epoch_loss_L1) / len(epoch_loss_L1), epoch)
        writer.add_scalar('TrainLoss/total_loss/' + str(self.args.client_sites[idx]), sum(epoch_loss) / len(epoch_loss),epoch)
        writer.add_scalar('TrainLoss/adv_g/' + str(self.args.client_sites[idx]), sum(epoch_loss_adv_g) / len(epoch_loss_adv_g), epoch)
        writer.add_scalar('TrainLoss/adv_d/' + str(self.args.client_sites[idx]), sum(epoch_loss_adv_d) / len(epoch_loss_adv_d), epoch)
        torch.cuda.empty_cache() # !!!
