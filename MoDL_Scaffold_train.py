...
def get_args():
    ...
    parser.add_argument('--client_pats' , nargs='+', default=[5, 5], type=int, help='Vector of client samples (patients, 10 slices each)')
    parser.add_argument('--client_sites', nargs='+', default=[2, 5], type=int, help='Vector of client sites (local data distribution)')
    parser.add_argument('--share_int'   , '--share_int', type= int, default=100, help='how often do we share weights(measured in steps)')
...
global_dir = 'Results/federated/client%s_clients%d_sites%s/personalLam%d/\
Scaffold/clientLR%.1e_client%s/sync%d/%s_pool%d_ch%d/train_pats_%s/seed%d' % (
      client_opt, num_clients,
      site_string, per_lam,
      lr, client_opt,
      share_int, hparams.img_arch,
...
# Initialize scratch corrector
scratch_corrector = MoDLDoubleUnroll(hparams)
scratch_corrector = scratch_corrector.cuda()
# Zero everything
local_state = scratch_corrector.state_dict()
for key in local_state:
    local_state[key] = torch.zeros(local_state[key].shape, device='cuda')
scratch_corrector.load_state_dict(local_state)
...
for i in range(num_clients):
    ...
    client_corrections.append(copy.deepcopy(local_model))
    
    # Client correction updates
    local_model = MoDLDoubleUnroll(hparams)
    local_model = local_model.cuda()
    # No train needed
    local_model.eval()
    ...
    for key in local_state:
        local_state[key] = torch.zeros(local_state[key].shape, device='cuda')
    local_model.load_state_dict(local_state)
    client_corr_updates.append(copy.deepcopy(local_model))
...
download_cfile = local_dir + '/fed_c_download.pt'
upload_files   = [local_dir + '/fed_upload%d.pt' % idx for \
                  idx in range(num_clients)]
upload_c_files = [local_dir + '/fed_c_upload%d.pt' % idx for \
                  idx in range(num_clients)]

# For each training-communication round
for round_idx in range(num_rounds):
    # All clients download global correction model
    if round_idx > 0:
        saved_corr_model  = torch.load(download_cfile)
        global_corr_state = copy.deepcopy(saved_corr_model['model_state_dict'])
    else:
        # Zeroes
        global_corr_state = copy.deepcopy(scratch_corrector.state_dict())
    ...
    for client_idx in range(num_clients):
        if round_idx == 0:
            ...
            client_models[client_idx].load_state_dict(saved_model['model_state_dict'])
        else:
            # 'Download' weights and correctors from server
        ...
        # Store original downloaded weights
        global_model_state = copy.deepcopy(saved_model['model_state_dict'])

        # Local updates performed by client for a number of steps
        # Fetch correctors once
        local_corr_state  = client_corrections[client_idx].state_dict()
        update_corr_state = client_corr_updates[client_idx].state_dict()
        ...
        for sample_idx in tqdm(range(share_int)):
            ...
            torch.nn.utils.clip_grad_norm(client_models[client_idx].parameters(), 
                                          hparams.grad_clip)
            optimizers[client_idx].step()
            
            # Perform correction
            local_state = client_models[client_idx].state_dict()
            for key in local_state.keys():
                local_state[key] = local_state[key] - \
                    hparams.lr * (global_corr_state[key] - local_corr_state[key])
            client_models[client_idx].load_state_dict(local_state)
...
Avg. RSS %.4f, Avg. Coils %.4f, Avg. NMSE %.4f' % (
        ...
        # After each round, save local weights for upload
        ...
        # Compute and upload difference corrector to server
        local_state = client_models[client_idx].state_dict()
        for key in local_state.keys():
            update_corr_state[key] = -global_corr_state[key] + \
                1/(share_int * hparams.lr) * (global_model_state[key] - local_state[key])
            local_corr_state[key]  = local_corr_state[key] + update_corr_state[key]
        # Update local correction
        client_corrections[client_idx].load_state_dict(local_corr_state)

        if os.path.exists(upload_c_files[client_idx]):
            os.remove(upload_c_files[client_idx])
        torch.save({'model_state_dict': update_corr_state},
                   upload_c_files[client_idx])
        ...
        if np.mod(round_idx + 1, save_interval) == 0:
            torch.save({
                ...
                'corr_state_dict': client_corrections[client_idx].state_dict(),
    ...
    with torch.no_grad():
        ...
        # Start with correction updates from first client
        saved_model = torch.load(upload_c_files[0])
        corr_diff_accumulator = saved_model['model_state_dict']
        
        # Accumulate corrections from all other clients
        for fed_idx in range(1, num_clients):
            # Only load the model once
            temp_model = torch.load(upload_c_files[fed_idx])
            for key in temp_model['model_state_dict'].keys():
                corr_diff_accumulator[key] = \
                    corr_diff_accumulator[key] + temp_model['model_state_dict'][key]
                    
        for key in corr_diff_accumulator.keys():
            # Average at the end, not sum
            corr_diff_accumulator[key] = corr_diff_accumulator[key]/num_clients

        # Perfom update of correction using average differences
        for key in corr_diff_accumulator.keys():
            global_corr_state[key] = \
                global_corr_state[key] + corr_diff_accumulator[key]
        # Clients will download this
        torch.save({'model_state_dict': global_corr_state}, download_cfile)

    # Save global models periodically
    if np.mod(round_idx + 1, save_interval) == 0:
        torch.save({
            'round': round_idx,
            'model_state_dict': state_dict_accumulator,
            'corr_state_dict': client_corrections[client_idx].state_dict(),
            'optimizer_state_dict': optimizers[client_idx].state_dict(),
            'scheduler_state_dict': schedulers[client_idx].state_dict(),
            'client_files': global_client_files[client_idx],
