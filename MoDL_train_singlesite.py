...
# For each epoch
    for epoch_idx in range(start_epoch, end_epoch):
        ...
        for sample_idx, sample in tqdm(enumerate(train_loader)):
           ...
           # Verbose
            print('Epoch %d, Site %d ,Step %d, Batch loss %.4f. Avg. SSIM %.4f, Avg. RSS %.4f, Avg. Coils %.4f, Avg. NMSE %.4f' % (
                epoch_idx, sites[0], sample_idx, loss.item(), running_ssim, running_loss, running_coil, running_nmse))
        ...
        torch.save({
            ...
            'loss': loss,
            'hparams': hparams,
            'train_mask_params': train_mask_params}, local_dir + '/ckpt_epoch' +str(epoch_idx)+ '_site'+str(sites[0])+'last_weights.pt')
        ...
        if args.comp_val:
            ...
            saved_model = torch.load(local_dir + '/ckpt_epoch' +str(epoch_idx)+ '_site'+str(sites[0])+'last_weights.pt')
